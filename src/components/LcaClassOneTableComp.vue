<template>
  <div class="badge-display">
      <span class="custom-badge">{{ countDisplay }}</span>
  </div>
...
const filteredLogStatus = computed(() => {
  const statusArray = Array.isArray(logStatus.value) ? logStatus.value : [];
  return statusArray.filter(status => status?.department === 'Life Choices Academy C1');
});
